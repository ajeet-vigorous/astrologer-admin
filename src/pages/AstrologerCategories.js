...
const AstrologerCategories = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editData) {
        await astrologerCategoryApi.edit({ filed_id: editData.id, name: form.name, image: imageData || undefined });
      } else {
        await astrologerCategoryApi.add({ name: form.name, image: imageData });
      }
      setShowModal(false);
      setEditData(null);
      setForm({ name: '' });
  ...
  const handleStatusToggle = async (id) => {
    try {
      await astrologerCategoryApi.status({ status_id: id });
      fetchData();
    } catch (e) {
      console.error(e);
    }
    ...
    <div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editData ? 'Edit Category' : 'Add Category'}>
        <form onSubmit={handleSubmit}>
          <FormInput label="Category Name" name="name" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required />
