...
const YearlyHoroscope = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error('Error fetching yearly horoscopes:', err);
      setData([]);
    }
    setLoading(false);
  };
    ...
    <div>
      ...
      <div className="cust-topbar">
        <div className="cust-topbar-left">
          ...
          <div>
            <h2 className="cust-title">Yearly Horoscope</h2>
            <div className="cust-count">{totalRecords} total</div>
          </div>
        </div>
        <div className="cust-topbar-right">
          <button onClick={async () => { try { const r = await horoscopeApi.generateYearly(); toast.success(r.data?.message || 'Yearly horoscope generated!'); fetchData(); } catch(e) { toast.error('Generation failed'); } }} className="cust-btn cust-btn-success">
            <Star size={15} /> Generate Yearly
          </button>
        </div>
      </div>
      ...
      <div className="cust-card">
        {loading ? <Loader text="Loading yearly horoscopes..." /> : (
